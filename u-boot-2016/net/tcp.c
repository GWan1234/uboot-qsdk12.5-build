...
static u16 tcp_checksum_compute(void *data, int len, __be32 sip, __be32 dip)
{
		...
		u32 tcphdr_len, u8 *ethaddr, tcp_conn_cb cb)
{
	...
	if (c == &tmp_c) {
		/* malloc fail, try to send TCP_RST to peer and return NULL */
	...
	tcp_send_packet_opt(c, TCP_SYN | TCP_ACK, c->local_seq, c->peer_seq + 1,
...
int tcp_connect(__be32 destip, __be16 port, tcp_conn_cb cb, void *pdata)
{
	struct tcp_conn *c;
	u16 localport;
	...
	c = calloc(1, sizeof(struct tcp_conn));
	if (!c)
		return -ENOMEM;

	list_add_tail(&c->node, &conn_head);

	if (tcp_port_seq >= 65535)
		tcp_port_seq = 50000;

	if (tcp_port_seq == 50000) {
		srand((u32)get_ticks());
		tcp_port_seq += (rand() * rand()) % 10000;
	}

	localport = tcp_port_seq++;
...
static void tcp_conn_fill(struct tcp_conn *c, struct tcp_hdr *tcp,
...
bool receive_tcp(struct ip_hdr *ip, int len, struct ethernet_hdr *et)
{
	...
	case SYN_SENT:
		if (!(flags & TCP_SYN)) {
			tcp_send_packet(c, TCP_RST, c->local_seq,
					    c->peer_seq, NULL, 0);
		...
		tcp_conn_fill(c, tcp, tcphdr_len, et->et_src);
	...
	case SYN_RCVD:
		...
		if (c->status == SYN_SENT) {
			tcp_send_packet(c, TCP_ACK, c->local_seq,
					    c->peer_seq, NULL, 0);
		}
...
static int tcp_rexmit_check(struct tcp_conn *c, struct tcp_cb_data *cbd)
{
	ulong curts, timeout;
...
static void tcp_connect_rexmit_reset(struct tcp_conn *c)
{
	c->ts = get_timer(0);
	c->num_rexmit++;
}

static int tcp_connect_rexmit_check(struct tcp_conn *c,
					struct tcp_cb_data *cbd)
{
	ulong curts, timeout;
	...
	if (get_timer(c->ts_rexmit) > TCP_REXMIT_MAX_CONN_DELAY) {
		...
		* resetting the connection
		*/
		tcp_send_packet(c, TCP_RST, c->local_seq, c->peer_seq,
				    NULL, 0);
	...
	timeout = c->ts + (TCP_CONNECT_INIT_DELAY << c->num_rexmit);
...
static void tcp_conn_check(struct tcp_conn *c)
{
	...
	case CONNECT:
		...
		tcp_send_packet_opt(c, TCP_SYN, c->local_seq,
	...
	case SYN_SENT:
		switch (tcp_connect_rexmit_check(c, &cbd)) {
		case -1:
			return;
		case 1:
			tcp_set_mss_opt(opt, c->mss);
			...
			void *opt, int opt_size, const void *payload, int payload_len)
{
	...
	len = IP_HDR_SIZE + TCP_HDR_SIZE + opt_size + payload_len;
	net_set_ip_header_tcp(pkt, c->ip_remote, net_ip, len, IPPROTO_TCP);
	pkt += IP_HDR_SIZE;
