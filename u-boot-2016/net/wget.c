...
static void wget_initiate(struct wget_pdata *pdata, struct tcp_cb_data *cbd)
{
	...
	/* send request header */
	tcp_send_data(cbd->conn, pdata->req_hdr, strlen(pdata->req_hdr));

	pdata->state = WGET_REQ_SENT;
}

static int wget_recv_data(struct wget_pdata *pdata, void *data, size_t len)
{
...
static int wget_parse_response(struct wget_pdata *pdata)
{
	...
	*fields_ptr = 0;
	...
	if (simple_strtoul(p + 1, NULL, 10) != 200) {
		pdata->result = -1;
		return -1;
	}
	...
	if (p) {
		...
		pdata->data_len = simple_strtoul(p, NULL, 10);
		printf("Content-Length: %zu\n", pdata->data_len);
	} else {
		if ((ulong)pdata->data_ptr >= CONFIG_SYS_SDRAM_BASE &&
...
static void wget_rx(struct wget_pdata *pdata, struct tcp_cb_data *cbd)
{
	char *new_ptr;
	int ret;
...
static void wget_tcp_callback(struct tcp_cb_data *cbd)
{
...
int start_wget(ulong load_addr, const char *url, size_t *retlen)
{
	...
	if (*path == ':') {
		if (path[1] == '/') {
			...
			val = simple_strtoul(path + 1, &p, 10);
