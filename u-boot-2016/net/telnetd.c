...
static size_t telnetd_build_greeting(char *buf, size_t buf_sz)
{
	...
	memcpy(buf + off, telnet_greeting_prefix, sizeof(telnet_greeting_prefix));
	off += sizeof(telnet_greeting_prefix);

	n = snprintf(buf + off, buf_sz - off, "Version: %s\r\n", version_string);
	if (n < 0 || (size_t)n >= buf_sz - off)
		return 0;
	off += n;

	n = snprintf(buf + off, buf_sz - off, "Ported from project: Yuzhii0718/bl-mt798x-dhcpd\r\n");
	if (n < 0 || (size_t)n >= buf_sz - off)
		return 0;
	off += n;

	n = snprintf(buf + off, buf_sz - off, "Author: Yuzhii0718\r\n\r\n%s", prompt);
	if (n < 0 || (size_t)n >= buf_sz - off)
		return 0;
	off += n;
...
static void telnetd_send_or_queue(struct tcp_cb_data *cbd,
				   ...
				   char *buf, u32 len)
{
	...
	ret = tcp_send_data(cbd->conn, buf, len);
	if (ret) {
		pdata->outbuf = buf;
 ...
 */
static void telnetd_flush_edit_outbuf(struct tcp_cb_data *cbd,
				      struct telnetd_pdata *pdata)
{
	if (!pdata->edit_outbuf_len)
		return;

	if (!tcp_send_data(cbd->conn, pdata->edit_outbuf,
 ...
 */
static void telnetd_process_input(struct tcp_cb_data *cbd)
{
	struct telnetd_pdata *pdata = cbd->pdata;
	u32 i;
...
static void telnetd_callback(struct tcp_cb_data *cbd)
{
	struct telnetd_pdata *pdata;
	u8 sip[4];
	...
	case TCP_CB_NEW_CONN:
		...
		cbd->pdata = pdata;
		tcp_conn_set_pdata(cbd->conn, pdata);
		 ...
		 */
		{
			...
			if (fallback) {
				...
				if (!tcp_send_data(cbd->conn, telnet_greeting_fallback,
						      sizeof(telnet_greeting_fallback))) {
					pdata->outbuf = NULL;
					pdata->outbuf_len = 0;
					pdata->outbuf_pending = false;
	...
	case TCP_CB_DATA_SENT:
		...
		if (pdata->state == TELNETD_S_RESPONDING) {
			if (pdata->outbuf_pending) {
				if (!tcp_send_data(cbd->conn, pdata->outbuf,
						      pdata->outbuf_len)) {
					pdata->outbuf_pending = false;
					return;
				}
...
int telnetd_start(u16 port)
{
	...
	if (tcp_listen(htons(port), telnetd_callback))
		return -EIO;
...
void telnetd_stop(void)
{
	...
	tcp_listen_stop(htons(telnetd_inst.port));
...
static int do_telnetd(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	if (argc < 2)
